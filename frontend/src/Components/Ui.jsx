...
const Ui = () => {
  ...
  if (lang === "java") {
    // No replacement of System.out.print → System.out.println
    // You can still add System.out.flush() if needed for prompt visibility:
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      cleaned.push(line);
      if (
        /System\.out\.print\s*\(.*\)\s*;/.test(line) &&
        !lines[i + 1]?.includes("System.out.flush()")
      ) {
        cleaned.push("System.out.flush();");
      }
    }
 ...
 const handleInputSubmit = (e) => {
  ...
  if (promptIndex + 1 < currentPromptCount) {
    ...
    axios
      ...
      .then((res) => {
        // ✅ Display only what backend sends — no extra echo
